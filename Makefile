...
.PHONY: applets vfs-image flash-vfs flash clean
.PHONY: qemu

applets vfs-image:
	bash -lc 'source $(IDF_EXPORT) >/dev/null 2>&1 && $(PY) tools/applets.py build $(if $(DIST),--dist $(DIST),) $(if $(TARGET),$(TARGET),)'

flash-vfs:
	bash -lc 'source $(IDF_EXPORT) >/dev/null 2>&1 && $(PY) tools/applets.py flash $(if $(DIST),--dist $(DIST),) $(if $(PORT),--port $(PORT),) $(if $(BAUD),--baud $(BAUD),)'

flash:
	bash -lc 'source $(IDF_EXPORT) >/dev/null 2>&1 && $(PY) tools/applets.py build $(if $(DIST),--dist $(DIST),)'
...
qemu:
	...
	bash -lc 'source $(IDF_EXPORT) >/dev/null 2>&1 && MAGNOLIA_BUILD_DIR=build/qemu $(PY) tools/applets.py build $(if $(DIST),--dist $(DIST),)'
...
clean:
	rm -rf build
	find applets -maxdepth 3 -type d -name build -prune -exec rm -rf {} +
