...
#if defined(__NEWLIB__) || defined(__GLIBC__) || defined(__UCLIBC__) || defined(__MUSL__)
#define VI_HAVE_MEMRCHR 1
#define VI_HAVE_STRCHRNUL 1
#define VI_HAVE_STRNCASECMP 1
#define VI_HAVE_ISBLANK 1
#endif

#if defined(ESP_PLATFORM) || defined(__XTENSA__) || defined(__riscv)
#define VI_NO_TCDRAIN 1
#endif

#if defined(TIOCGWINSZ)
#define VI_HAVE_TIOCGWINSZ 1
#endif
...
#define CTYPE_CAST(c) ((unsigned char)(c))

#if defined(VI_NO_TCDRAIN)
static int vi_tcdrain(int fd)
{
    (void)fd;
    return 0;
}
#define tcdrain vi_tcdrain
#endif
...
#if !defined(VI_HAVE_MEMRCHR)
static void *vi_memrchr(const void *s, int c, size_t n)
{
    if (s == NULL || n == 0) {
        return NULL;
    }
...
#define memrchr vi_memrchr
#endif

//
// Return pointer to either the found character or the null terminator
//
#if !defined(VI_HAVE_STRCHRNUL)
static char *vi_strchrnul(const char *s, int c)
{
    if (s == NULL) {
        return NULL;
    }
...
//
#if !defined(VI_HAVE_ISBLANK) && !defined(isblank)
static int vi_isblank(int c)
{
    return (c == ' ' || c == '\t');
}
#define isblank vi_isblank
#endif

#if !defined(VI_HAVE_STRNCASECMP)
static int vi_strncasecmp(const char *s1, const char *s2, size_t n)
{
    if (s1 == NULL || s2 == NULL) {
        return 0;
    }
...
#define strncasecmp vi_strncasecmp
#endif
...
{
    // skip WS
    while (isspace(CTYPE_CAST(*dot)) && *dot != '\n' && dot < end - 1)
        dot++;
}
...
static int find_range(char **start, char **stop, char c)
{
    ...
    if (strchr("cdy><", c)) {
        ...
        if (dot > text &&
            ((isspace(CTYPE_CAST(dot[-1])) && !isspace(CTYPE_CAST(dot[0]))) ||
             (ispunct(CTYPE_CAST(dot[-1])) && !ispunct(CTYPE_CAST(dot[0]))) ||
             (isalnum(CTYPE_CAST(dot[-1])) && !isalnum(CTYPE_CAST(dot[0])))))
...
static int st_test(char *p, int type, int dir, char *tested)
{
    ...
    if (type == S_OVER_WS) {
        c = c0;
        test = ((isspace(CTYPE_CAST(c))));
    }
    if (type == S_END_PUNCT) {
        c = ci;
        test = ((ispunct(CTYPE_CAST(c))));
    }
    if (type == S_END_ALNUM) {
        c = ci;
        test = ((isalnum(CTYPE_CAST(c))) || c == '_');
    }
    *tested = c;
    return test;
}
...
static char readit(void) // read (maybe cursor) key from stdin
{
    ...
    if (n <= 0) {
        ...
        fflush(stdout);
        n = (int)safe_read(STDIN_FILENO, readbuffer, sizeof(readbuffer));
        if (n < 0) {
        ...
        if (readbuffer[0] == 27) {
            ...
            while (awaitInput(ticsPerChar)) {
                ...
                ssize_t r = safe_read(STDIN_FILENO, readbuffer + n, sizeof(readbuffer) - (size_t)n);
                if (r <= 0)
                    break;
                n += (int)r;
                if (n > (int)sizeof(readbuffer) - 8)
                    break;
            }
        }
        chars_to_parse = n;
    ...
    if (c == 27 && n > 1) {
        ...
        for (eindex = esccmds; eindex < &esccmds[ESCCMDS_COUNT]; eindex++) {
            int cnt = (int)strnlen(eindex->seq, sizeof(eindex->seq));
            if (n <= cnt)
                continue;
...
found:
    ...
    chars_to_parse -= n;
    memmove(readbuffer, readbuffer + n, sizeof(readbuffer) - (size_t)n);
    return c;
}
...
key_cmd_mode:
    ...
    case '"': // "- name a register to use for Delete/Yank
        c1 = get_one_char();
        c1 = (char)tolower(CTYPE_CAST(c1));
        if (islower(CTYPE_CAST(c1))) {
            YDreg = c1 - 'a';
        } else {
            indicate_error(c);
        }
    ...
    case 'W': // W- forward a blank-delimited word
        ...
        if (c == 'B')
            dir = BACK;
        if (c == 'W' || isspace(CTYPE_CAST(dot[dir]))) {
    ...
    case 'J':                // J- join current and next lines together
        ...
        if (dot < end - 1) { // make sure not last char in text[]
            *dot++ = ' ';    // replace NL with space
            file_modified++;
            while (isblank(CTYPE_CAST(*dot))) { // delete leading WS
                dot_delete();
            }
        }
        end_cmd_q(); // stop adding to q
    ...
    case 'e': // e- end of word
        ...
        if ((dot + dir) < text || (dot + dir) > end - 1)
            break;
        dot += dir;
        if (isspace(CTYPE_CAST(*dot))) {
            dot = skip_thing(dot, (c == 'e') ? 2 : 1, dir, S_OVER_WS);
        }
        if (isalnum(CTYPE_CAST(*dot)) || *dot == '_') {
            dot = skip_thing(dot, 1, dir, S_END_ALNUM);
        } else if (ispunct(CTYPE_CAST(*dot))) {
    ...
    case 'w':                               // w- forward a word
        if (isalnum(CTYPE_CAST(*dot)) || *dot == '_') { // we are on ALNUM
            dot = skip_thing(dot, 1, FORWARD, S_END_ALNUM);
        } else if (ispunct(CTYPE_CAST(*dot))) { // we are on PUNCT
    ...
    case '~': // ~- flip the case of letters   a-z -> A-Z
        if (islower(CTYPE_CAST(*dot))) {
            *dot = (char)toupper(CTYPE_CAST(*dot));
            file_modified++;
        } else if (isupper(CTYPE_CAST(*dot))) {
            *dot = (char)tolower(CTYPE_CAST(*dot));
            file_modified++;
        }
        dot_right();
        end_cmd_q(); // stop adding to q
