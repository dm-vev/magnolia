...
static char *execs(char *ap, char *t[])
{
    ...
    int rc = 0;
    int ret = m_elf_run_file(p, count_argv(t), &t[0], &rc);
    if (ret == 0) {
        exitval = rc;
        done();
    }
    ...
    switch (err) {
    case ENOEXEC:
    case EINVAL: /* could be a shell script */
        funcnt = 0;
        flags = 0;
        *flagadr = 0;
        comdiv = 0;
...
void gocsh(char **t, char *cp, char **xecenv)
{
    ...
    (void)xecenv;
    int rc = 0;
    int ret = m_elf_run_file("/bin/csh", i + 1, newt, &rc);
    if (ret == 0) {
        exitval = rc;
        done();
    }
...
void await(int i, int bckg)
{
    ...
    while (pwc) {
        ...
        if (WIFSTOPPED(status)) {
            sig = 0177; /* ptrace! return */
            w_hi = WSTOPSIG(status);
        } else
#endif
        if (WIFSIGNALED(status)) {
            sig = WTERMSIG(status);
        } else if (WIFEXITED(status)) {
            w_hi = WEXITSTATUS(status);
        }
        if (sig) {
